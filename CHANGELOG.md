...
### Fixed
- **数据总览流程图放大填满面板，消除底部留白**：`data-flow-layout` 恢复 `flex: 1` 占满剩余视口高度（d51e9c7 改为 `flex: none` 后面板收缩为 SVG 固有高度，余下空间堆在页面底部，1280×720 下达 159px）；流程图 viewBox 从 1200×340 重排为 1000×396（宽高比 ≈2.53 贴近面板形状，环节圆 26→30，节点纵横间距重排），渲染比例 0.96→1.15（文字实际放大约 20%）；SVG `max-height` 改 `100%`——宽度优先拉满、高度受限时等比收缩，图始终填满面板的受限方向（1280×720 实测填充 99.8% 宽 × 99.6% 高），wrap 上下 padding 对称化（20px）并补 `align-items: center` 居中。
...
- 预测系统挂钩学练段（路线 A + B）：打通"检测新事件 -> 预测新方向 -> 反馈用户"闭环。
  ...
  - 边界：预测仅作信息性前瞻提示，不改岗位版本、不绕过审核（temporal-system.md）。测试：后端 111 + e2e 25。
...
- 学练赛证"证/赛"段贯通到匹配与 API：data/CERTS.yml（15 张权威证书：osta 标准 6 +
  ...
  高教学会目录 5 + 讯飞大赛 5 + 天池/DF 3，含获奖等级换算）两个人工审定映射层；
  backend/matching/xlzsz.py 域模块（反查/正查/换算，零 LLM）；匹配引擎 judge()
  新增证书证据分支（证书命中 -> 已具备，证据可回链颁发机构）与竞赛证据分支
  （竞赛经历 -> 缺失提升部分具备）；learning_path() 赛/证段从模板改为真实实体
  推荐；training-output 新增 recommended_certs；新端点 GET /api/v1/xlzsz/certs
  与 /contests（contracts.md 已登记）；migration 0009 三表 + scripts/xlzszdb.py
  导入（本地 PG 入库 2050 证书 + 1199 竞赛 + 569 标准工作要求）；测试 +17。
- 学练赛证"证/赛"段实体数据首批入库（补齐四段中唯一零数据的两段）：调研文档
  `docs/research/xlzsz-channels.md` 实测 10 个渠道（8 个公开 JSON API 无鉴权）；
  采集脚本三件套——`certget.py`（osta 国家职业标准库 702 条 + 教育部 1+X 证书
  1240/标准 450 + 华为认证 108，归一 `cert-catalog.jsonl` 共 2050 条）、
  `raceget.py`（讯飞 AI 开发者大赛 629 + 天池 270 + DataFountain 300，归一
  `race-catalog.jsonl` 共 1199 场）、`certparse.py`（10 份数字技术域标准 PDF 的
  工作要求表结构化：569 工作内容 / 3131 能力要求 / 3404 知识要求，含等级分离，
  列优先 X 簇间隙分界 + 编号深度切分，零 LLM 确定性解析）；
  data/SOURCES.yml 登记两个新来源通道（cert-catalog / race-catalog）。
- 求职工作区串联"目标岗位 → 我的画像 → 人岗诊断 → 学习路径 → 投递简历"：选择岗位会保存当前目标，画像首屏收敛为常用技能并修复新增能力证明，诊断改用同口径的岗位能力/投递基础/证明/缺口摘要，学习路径压缩为行动网格并衔接简历；简历自动带入当前画像、能力证明与目标岗位，岗位建议首屏可见，并修复真实 PDF 响应读取。
- 评测质量左侧“评测对象”改为真正可切换的导航：岗位映射、领域判定、事件抽取分别展示自身准确率、复盘数量和全部样本详情，趋势回测独立展示预测矩阵；补充 100/50/30 条冻结样本的评测结论下钻，避免不同口径指标混在同一界面。
- Hiro 品牌标志接入 Web 应用壳层：导航使用“证据之桥 H”组合标，Next 原生站点图标同步更新。
- 评测质量的预测偏差改为业务化展示：用“预测上升，实际下降”等文字替代 `flat->down` 机器标签，增加 3×3 判断结果矩阵、偏差分类，以及 136 条具体回测记录下钻。
- 简历批量解析增加 `--fallback-only`：模型服务不稳定时按24个逻辑案例选择代表文件，用技能词典生成基础画像并向同案例PDF/DOCX/TXT格式传播；72份测试档案现已全部具备画像，界面区分模型解析、词典解析和共享画像。`dbimport` 同步改为按最新档案去重并更新 PostgreSQL 画像，而不是对追加历史逐行计数且冲突跳过。
- 数据集完整档案路由 `/data/assets/{dataset_id}`：统一提供概览、版本、血缘、字段质量和记录视图。简历档案已接入全部记录浏览，支持样本性质/格式/解析状态筛选、每页 50 条、合成样本标记，以及单份简历的结构化画像、原始文档预览和处理信息；原 `/resumes` 保留为上传与解析工具入口。
- 证据库来源改为可搜索选择框，由 `/evidence/facets` 返回真实来源及记录数；列表隐藏技术证据 ID，质量由裸百分比改为绿/黄/红的高、中、低等级，精确分值保留在悬浮提示和详情中。
- 证据库新增发布时间范围筛选，支持 `date_from` / `date_to` 后端过滤；来源、类型、审核状态、时间和关键词任一条件变化后 300ms 自动刷新并回到第一页。
- 比赛版数据工作区收敛为五个正式界面：总览、数据资产、证据审核、评测质量、趋势洞察。新增数据资产双视图与数据集版本/来源详情、14,174 条证据的后端分页浏览、运行配置/事件/产物详情；市场信号、技术传导、当前预测与岗位影响合并为单页能力域洞察，岗位影响转审核任务、预测质量归评测页，删除五个重复时间子路由与组件。
- 数据集记录口径分层：`DatasetItem` 新增 `count_scope` 与 `stage_counts`，招聘岗位主记录数改用 8,656 条已解析入库 JD，并单独保留 70 条当前标准化样本；`dbimport` 后续登记同步改用完整 `jd-parsed.jsonl`，避免比赛展示把局部样本误当总数据规模。
- 数据治理详情 API：新增数据集版本与 manifest 摘要、来源通道证据统计、证据分页检索、流水线单次运行详情四组只读能力；历史招聘证据无法从发布平台可靠反推采集通道时返回明确的 `attribution=unavailable`，不生成重叠统计。
- 时间情报导航与信号范围筛选：市场信号新增全部时间、近 7/30/90 天和近 1 年筛选，能力域计数随时间窗联动；新增完整信号列表 API，移除信号页 500/2000 条硬截断，完整历史信号按唯一 `signal_id` 返回。
- 数据来源通道二级追溯：`/data/sources` 右侧抽屉中的来源卡支持整卡点击与键盘进入，在同一抽屉内查看来源类型、覆盖时间、采集模式、来源说明和使用许可；补齐返回、关闭、焦点闭环与关闭后的焦点恢复，桌面和窄屏共用同一交互层级，不虚构尚未接入的通道级统计。
- 趋势预测页对齐与密度重构：移除无数据意义的热度百分比进度条和空白大卡片，将当前能力域收敛为热度、环比、置信度、阶段四项摘要；预测列表补充稳定列头和数值右对齐，能力域列自适应宽度，整行支持点击切换，并适配窄屏双行布局。
- 市场信号与技术传导界面收口：移除与二级导航重复的页标题；市场信号新增全文搜索、类型筛选和可点击的高频能力域过滤，压缩单条信号元信息并优化窄屏筛选顺序；技术传导修复 API 蛇形字段未映射导致的全表空值与 `undefined 个月`，将 PyPI/npm 合并为生态包层，以四阶段传导路径呈现论文先行、岗位先行和链路不完整三类关系，并支持分类筛选。
- 预测复盘页重构为结论优先的信息层级：首屏明确标示改进规则是否达到简单基线，用同尺度横条集中比较 30/60/90 天旧规则、改进规则与基线，移除重复的六个运行按钮和四张指标卡；错误分布支持按预测周期切换，补充错误占比与严重程度表达，运行 ID、数据集和规则版本收纳到折叠详情，并完成桌面与 390px 窄屏适配。
- 时间情报工作区可读性与完整性收口：路由级共享导航统一五个子页，内部能力/状态/关系枚举改为中文展示，市场信号由静默截断 20 条改为每批 50 条渐进加载；API 与离线 View Model 的近 90 天信号上限由 500 提升到 2000，覆盖当前 1749 条全集；修复市场信号滚动容器和预测复盘错误标签/次数重叠。
...
- 招聘区交互打磨（P2）：
  - 岗位更新页删除无动作的「版本说明」按钮；「岗位/基准版本」单选项 Select 改为静态展示文本；审核构成与发布记录的标签对齐设计词表（已确认/待审核/证据不足）。
  - 状态词表对齐 design-recruiting.md：`reviewLabels` 从 已接受/待审/待确认 改为 已确认/待审核/证据不足；候选人技能改用候选人语义（已具备/部分具备/缺失），不再复用审核词表。
...
- 岗位更新页「分析变化」不再清空已有审核结果（原逻辑分析完成后重置为初始状态）。
- 岗位更新页「版本对比」原为写死「必备技能 7→8 / 加分技能 5→4」且与真实数据不符，现按变化清单实时计算新增/修改/删除数。
- 趋势洞察继续压缩文案：能力域数量与信号数合并为一行；当前判断区严格收敛为“判断 + 置信度”和“范围 + 信号数 + 岗位影响 + 截止日期”两行，删除解释段落与筛选说明，关键数字使用等宽字和更高字重。
- 趋势洞察改为固定工作台视口，外层页面不再上下滚动；能力域列表、分析内容和岗位影响三栏分别接管纵向滚动，窄屏由内容区内部滚动并保持顶部导航稳定。
- 趋势洞察删除与左侧列表重复的右上能力域选择器；时间筛选改名“信号范围”，并明确其只影响能力域排行、信号数和近期依据。当前判断标注为最近一次预测结果，技术传导标注为完整历史，避免用户误以为切换时间会重新计算预测。
- 趋势洞察计算口径移除用户可见的 `onset` 术语，统一说明为“首次达到规模阈值的月份”；前端同时兼容替换旧 API 文案。
- 趋势信号统一使用唯一 `signal_id` 口径：7,474 行物理产物去除 1,344 条重复后为 6,130 条逻辑信号；文件模式、Mock、数据集阶段统计和 `dbimport` 日志与 PostgreSQL 主键口径保持一致。
- 趋势洞察移除 Top 10 截断，左侧与选择器覆盖完整 30 个能力域并显示零信号状态；标题明确当前时间范围信号数与 6,130 条唯一历史信号。无预测能力域改为“暂无预测”，岗位影响显示真实岗位名称并将 `jd_preceded` 转为可读说明，同时删除重复的方向标签。
- 证据审核默认入口改为证据库，视图顺序调整为“证据库 / 审核任务”；趋势岗位影响和评测待办使用 `?view=tasks` 显式进入任务视图。
- 审核任务降低方框密度：列表与详情合并为一个主容器，任务项改用分隔线，判断问题、依据和审核结果取消独立卡片边框；“需要你判断”缩为“需判断”，桌面端与具体问题保持同一行。
- 审核任务从原始字段堆叠重构为“需要判断什么 → 系统建议 → 判断依据 → 做出判断”：后端补回系统映射岗位、映射方式、领域判断理由和事件字段；列表隐藏来源/运行 ID，完成任务只展示审核结果，技术元数据收进折叠详情，并增加待处理/已完成筛选。
- 证据审核的证据库接管 `workflow-page` 剩余高度并启用纵向滚动，修复 50 条分页表格被父层 `overflow: hidden` 裁切、鼠标滚轮无法上下浏览的问题。
- 趋势洞察加载态移除重复嵌套的 `AppShell`，不再在内容区渲染第二套放大导航；能力域标题、排行与选择器只显示中文映射名称，`cap_xx` 留在内部数据和技术详情中。
- 数据总览流转图移除 `55vh` 强制高度，容器改由 SVG 宽高比自然撑开，消除宽屏下关系图上下的大块无效留白。
- 数据总览删除与顶部导航重复的“查看数据资产 / 处理证据审核 / 查看趋势洞察 / 查看评测质量”快捷入口，总览只保留状态指标和数据流关系。
- 删除 `/datasets`、`/quality`、`/data/quality` 与 `/career` 四个纯重定向兼容页面；数据资产、评测质量和求职入口只保留 `/data`、`/evaluation`、`/career/jobs` 三套正式路径，应用壳不再识别废弃路径。
