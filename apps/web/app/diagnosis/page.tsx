...
async function fetchDiagnosisServer(
  ...
  return apiFetch<DiagnosisView>(
    `/diagnosis/${encodeURIComponent(candidate || DEFAULT_CANDIDATE)}`,
  );
}
...
export default async function DiagnosisPage({
  ...
  searchParams: Promise<{ state?: string; candidate?: string }>;
}>) {
  const { state, candidate } = await searchParams;
  const variant = state === "empty" || state === "error" ? state : "ready";
  return (
    <DiagnosisWorkbench
      fixture={await fetchDiagnosisServer(variant, candidate)}
      state={variant}
    />
  );
}
