export type DashboardOverview = {
  ...
  jobs: Array<{ title: string; version: string; status: string; pending: number; href: string }>;
  activities: Array<{ label: string; detail: string }>;
  metrics: { positions: number; needs_update: number; pending_changes: number; published_versions: number };
  trends: Array<{ skill_id: string; label: string; months: string[]; values: number[]; sample_counts: number[] }>;
