...
test("adds / edits / removes project with audit trail", async ({ page }) => {
  ...
  await expect(page.locator(".project-audit-meta")).toContainText("已记录 1 条");
  ...
  await page
    .locator(".project-row-editing button.ant-btn-primary")
    .click();
  await expect(page.getByText("e2e 编辑项目")).toBeVisible();
  await expect(page.locator(".project-audit-meta")).toContainText("已记录 2 条");
  ...
  await expect(page.locator(".project-audit-meta")).toContainText("已记录 3 条");
});
...
test("renders empty and error variants", async ({ page }) => {
  ...
  await expect(page.getByText("候选人或岗位版本数据暂时不可用。")).toBeVisible();
});