...
状态：**未开始（审计发现的最大缺口）**。实测同日同标题事件 1428 组 ≈ 1438/8350（17%）为速览/正文重复，集中于 2026-02 后——信号幅度被结构性抬高，影响 D8 特征与回测、提前量的 2026 段信号强度（启动月判定影响较小）。去重键设计：同日 + 标题相似 + 实体重合。
...
状态：未开始。review-labels.csv 是标注包不是评测集（无标准答案与指标脚本）。
