...
### 涌现扫描器：新岗位自动发现（2026-08-31，`scripts/emergscan.py`）

把主案例 1 的人工论证方法通用化为零 LLM 自动探测器，每天定时链自动跑：
- 判据：近 90 天 ≥10 条 AND 增长比 ≥1.5（或从无到有）AND 标题变体 ≥3 AND 跨平台 ≥2；
  Jaccard ≥0.5 合并同方向，地名/职能泛词/已知领域词子串去噪。
- 验证样本 FDE：190 条 / 12 平台 / 117 标题变体 / 增长 16.6 倍，无人工干预自动检出。
- 前沿实验室源扩展（OpenAI/xAI/ElevenLabs 等 10 家 +1,626 条）后新检出：
  post training（20 倍）/ ai infrastructure / compute infrastructure /
  cloud infrastructure / data platform（21 倍）等方向。
- 产物 emerging-roles.json → /new-jobs 前端候选（月度趋势图 + 近期 JD 原文链接 +
  内容画像五要素 + 接受/拒绝审核流，人工审核后才可成为岗位草稿）。
