...
- 补抓通路（2026-08-25 打通）：`scripts/jdserve.py`（远程浏览器栈：Xvfb+Chrome/CDP+noVNC）→ 用户 noVNC 登录并手动搜索一次"焐热"WAF 信誉 → `scripts/jdauto.py run/detail` 在同一浏览器内自动抓搜索层与详情（冷浏览器/纯 cookie 注入均被软封锁，实测唯一可行通路）；`scripts/jdlisten.py` 为手动浏览时的被动监听兜底。已抓 6 关键词 115 个去重职位（100% 带发布日期），详情批抓进行中。
