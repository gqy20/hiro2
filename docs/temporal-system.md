...
| 职业标准/Excel | 能力本体和专家基线 | 不是历史时点 ground truth |
...
v2 在三 horizon 一致改进（h30 +5.0 / h60 +14.0 / h90 +12.8 点），仍逊平基线如实保留在复盘页。规则阈值必须来自错误分布的结构特征，禁止逐点调参刷分。`backtest.py --rule` 可重放任一版本，产物按规则版本隔离（`backtest-h{H}-r{N}.json`），复盘页提供 v1/v2 对比。
...
CLI 与 HTTP API 使用同一 Application Use Case；每次运行保存 `run_id`、数据集版本、模型/Prompt/规则版本、输入输出和错误分类，保证调优可复现。
